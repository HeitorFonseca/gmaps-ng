                ...
                if (!req.body.tipo) {
                    ...
                    user.save((err) => {
                        ...
                        if (err) {
                            ...
                            res.status(201).json({ id: user._id, nome: user.nome, email: user.email, tipo: user.tipo }); // Return success
                        }
                    });
                }
            }
...
router.put('/:id', function (req, res, next) {
    ...
    User.findOneAndUpdate(query, req.body, function (err, user) {
        if (err) {
            ...
            res.status(200).json({ id: user._id, nome: user.nome, tipo: user.tipo }); // Return success
        }
    });
});
...
router.get('/propriedades', permissions.requireToken, function (req, res, next) {
    ...
    jwt.verify(token, config.secret, function (err, decoded) {
        ...
        if (decoded.type == "produtor") {
            ...
            return res.status(500).send({ message: "Operação não permitida!" });
        }
        ...
        Property.aggregate([
            ...
            if (err) {
                ...
                res.status(500).json({ message: 'Não foi possivel retornar as propriedades. Erro: ', err });
            }
            else {
...
router.get('/propriedades/:propriedadeId', permissions.requireToken, function (req, res, next) {
    ...
    Property.aggregate([
        ...
        if (err) {
            ...
            if (!property) {
                res.status(404).json({ message: 'Não foi possivel retornar as propriedades. Erro: ', err });
            }
            else {
...
router.get('/propriedades/:propriedadeId/areas', function (req, res, next) {
    ...
    Area.findById(req.params.areaId, function (err, area) {
        if (err) {
            ...
            if (!area) {
                ...
                res.status(200).json({
                    id: area._id, propriedadeId: area.propriedadeId, nome: area.nome,
